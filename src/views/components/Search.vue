<template>
  <el-form ref="form">
    <el-form-item xlabel="Breeds">
      <el-select v-model="selected_id"  placeholder="Please Select Breed" size="large" :loading="loading" @change="selectOnChange" style="width:50%">
        <el-option
  ...
  <blockquote  v-show="emptyList && requestStatus != 'loading'" class='padding'>
    <div class='blockquote-content padding'>
      <p class='oblique'>
...
export default {
  ...
  mounted () {
    // Fetch the selected breed and bind to select input value.
    this.selected_id = sessionStorage.getItem('breed_id')
